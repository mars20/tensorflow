...
bool isNearlyEqual(const std::vector<float>& lhs, const std::vector<float>& rhs,
                   float max_abs_error) {
  std::map<int, float> mismatches;
  if (lhs.size() != rhs.size()) {
    printf("lhs and rhs have different sizes");
  }
  for (int i = 0; i < lhs.size(); ++i) {
    if (abs(lhs[i] - rhs[i]) > max_abs_error) {
      mismatches.emplace(std::make_pair(i, abs(lhs[i] - rhs[i])));
    }
  }
  if (!mismatches.empty()) {
    for (const auto& m : mismatches) {
      printf("Mismatch of %f at index %d \n", m.second, m.first);
    }
    printf("Test failed\n");
    return false;
  }
