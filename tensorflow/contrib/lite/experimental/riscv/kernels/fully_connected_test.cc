...
void TestFullyConnectedSimpleTest1() {
  ...
  std::vector<float> reference = {24, 25, 26, 58, 59, 60};
  CHECK(isNearlyEqual(result, reference) == true);
}

void TestFullyConnectedSimpleTest2() {
  FloatFullyConnectedOpModel m(/*units=*/1, /*batches=*/2,
  ...
  m.Invoke();
  std::vector<float> result = m.GetOutput();
  std::vector<float> reference = {11, 9};
  CHECK(isNearlyEqual(result, reference) == true);
}

void TestFullyConnected4Dinput() {
...
void TestFullyConnectedBlackBoxTest() {
  FloatFullyConnectedOpModel m(/*units=*/16, /*batches=*/2,
  ...
  for (int i = 0; i < input_sequence_size; i++) {
    ...
    expected.insert(expected.end(), golden_start, golden_end);
    std::vector<float> result = m.GetOutput();
    CHECK(isNearlyEqual(result, expected) == true);
  }
}
...
int main(int argc, char** argv) {
  tflite::fc_test::TestFullyConnectedSimpleTest1();
  tflite::fc_test::TestFullyConnectedSimpleTest2();
  tflite::fc_test::TestFullyConnected4Dinput();
  tflite::fc_test::TestFullyConnectedBlackBoxTest();
}
