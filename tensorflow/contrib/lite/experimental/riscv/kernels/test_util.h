...
==============================================================================*/
#ifndef TENSORFLOW_CONTRIB_LITE_EXPERIMENTAL_RISCV_KERNELS_TEST_UTIL_H_
#define TENSORFLOW_CONTRIB_LITE_EXPERIMENTAL_RISCV_KERNELS_TEST_UTIL_H_
...
// tolerance.
bool isNearlyEqual(const std::vector<float>& rhs, const std::vector<float>& lhs,
 ...
 private:
  ...
  std::pair<float, int32_t> QuantizationParams(float f_min, float f_max) {
    ...
    int32_t zero_point = std::min(
        q_max, std::max(q_min, static_cast<T>(round(q_min - f_min / scale))));
...
#endif  //TENSORFLOW_CONTRIB_LITE_EXPERIMENTAL_RISCV_KERNELS_TEST_UTIL_H_
