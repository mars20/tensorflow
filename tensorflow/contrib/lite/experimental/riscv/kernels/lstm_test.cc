...
void VerifyGoldens(const std::vector<std::vector<float>>& input,
  ...
  if(num_batches < 0){
    fprintf(stderr,"Number Batch should be greater than 0");
    exit(1);
  }
  const int num_inputs = lstm->num_inputs();
  if(num_inputs < 0){
    fprintf(stderr,"Number Inputs should be greater than 0");
    exit(1);
  }
  const int input_sequence_size = input[0].size() / num_inputs;
  if(input_sequence_size < 0){
    fprintf(stderr,"Number Inputs Sequence size should be greater than 0");
    exit(1);
  }
  for (int i = 0; i < input_sequence_size; ++i) {
    ...
    CHECK(isNearlyEqual(lstm->GetOutput(), expected, tolerance) == true);
  }
}

void TestNoCifgNoPeepholeNoProjectionNoClippingLstmTestLstmBlackBoxTest() {
  const int n_batch = 1;
  const int n_input = 2;
...
void TestCifgNoPeepholeNoProjectionNoClippingLstmTestLstmBlackBoxTest() {
  const int n_batch = 1;
  const int n_input = 2;
  // n_cell and n_output have the same size when there is no projection.
  const int n_cell = 4;
...
void TestNoCifgPeepholeProjectionNoClippingLstmTestLstmBlackBoxTest() {
  const int n_batch = 2;
  const int n_input = 5;
  const int n_cell = 20;
...
int main(int argc, char** argv) {
  tflite::lstm_test::TestNoCifgNoPeepholeNoProjectionNoClippingLstmTestLstmBlackBoxTest();
  tflite::lstm_test::TestCifgNoPeepholeNoProjectionNoClippingLstmTestLstmBlackBoxTest();
  tflite::lstm_test::TestNoCifgPeepholeProjectionNoClippingLstmTestLstmBlackBoxTest();
}
